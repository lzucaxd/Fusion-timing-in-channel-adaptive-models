"""
Evaluate HierBoCSetViT model per CHAMMI task splits (SD/OOD).
...
def plot_umap(embeddings, labels, title, save_path, metric='cosine'):
    ...
    reducer = umap.UMAP(
        ...
        min_dist=0.3, 
        n_epochs=500,
        metric=metric  # Use cosine distance
    )
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate HierBoCSetViT per CHAMMI task")
    ...
    parser.add_argument("--embed-dim", type=int, default=None, nargs='?', const=None, help="Auto-detected from timm ViT if not provided")
    ...
    parser.add_argument("--aggregator-depth", type=int, default=2)
    parser.add_argument("--aggregator-num-heads", type=int, default=3)
    
    # Checkpoint
    parser.add_argument("--checkpoint", type=str, 
                       default="./checkpoints/hier_boc_setvit_proxynca_128x128/checkpoint_latest.pth")
    
    # Output directory
    parser.add_argument("--output-dir", type=str, default="./evaluation_outputs_hier_boc")
    ...
    model.eval()
    print(f"Loaded HierBoCSetViT from epoch {checkpoint.get('epoch', 'unknown')}")
    
    datasets = ['Allen', 'HPA', 'CP']
    model_name = "hier_boc_setvit"
    ...
    for dataset_name in datasets:
        ...
        for task_col in task_cols:
            ...
            if metrics:
                print(f"{task_col:<15} {metrics['accuracy']:<12.4f} {metrics['macro_f1']:<12.4f}")
    ...
    if all_acc:
        ...
        print(f"Average Macro-F1: {np.mean(all_f1):.4f}")
